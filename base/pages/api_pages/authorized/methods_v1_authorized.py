...
class MethodsAuthorized:
    ...
    def post_v1_account_authorized():
        ...
        """
        auth_base = AuthBase(username="AniVar", password="Kykla1111!")
    ...
    def post_v1_account_user():
            """
            Метод для выполнения создание пользователя через POST-запрос на эндпоинт /Account/v1/User.

            Этот метод используется для создания пользователя, используя предоставленные имя пользователя и пароль.
            В процессе выполнения метода происходит несколько шагов:

            1. Создается экземпляр класса `AuthBase`, где задаются параметры создания (имя пользователя и пароль).
            2. Формируются данные для запроса и полный URL для создания.
            3. Выполняется отправка POST-запроса на сервер с предоставленными данными.
            4. Выполняется валидация ответа сервера, проверяется успешность создания.

            В случае успешного создания, результат валидации добавляется в отчет Allure и выводится в консоль.
            Если создание не удалась, тест завершится с ошибкой, и информация об этом также будет включена в отчет Allure.
            """
            auth_base = AuthBase(username="AniVar15", password="Kykla1111!15")
            ...
            with allure.step("Валидация ответа"):
                auth_base.validate_response_201(response, auth_base.validate_user_response)